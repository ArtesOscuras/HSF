...
**Context injection:**
- `_build_model_context()` in `app.py` includes the POC file listing (max 15 files) alongside dictionaries and hashcat rules.
- `check_inventory` tool in `tools.py` also lists POC files.

**System prompt:**
- Configured in `src/llm/config.py` under `prompts.poc_generation`. Instructs the LLM to generate Python 3.11+ scripts with:
  - Standard library only
  - Error handling with clear messages
  - Inline comments
  - Global variables for parameters (no argparse)
  - No dedicated listeners for RCE/reverse shells (HSF provides ports 8443 for no-root, 443 for root)
  - Filename as comment on first line
