...
- `detect_protection(status_code, headers, cookies)` — classifies the active challenge as `"akamai"`, `"cloudflare"`, or `None`. Detection is challenge-agnostic: Akamai is detected by the `AkamaiGHost` server header or an unsolved `_abck` cookie (flag `~-1~`); Cloudflare by the `cloudflare` server header with no `cf_clearance`.
- `SOLVED_PREDICATES` — a registry mapping each challenge type to a predicate that recognises the solved state (Akamai: `_abck` flag `~0~`; Cloudflare: `cf_clearance` present). Adding a new provider means adding a detection rule + a predicate.
