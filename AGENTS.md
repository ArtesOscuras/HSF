...
- `delete all` — wipes all operational data (machines, domains, users/credentials/passwords/hashes/tickets, people, shells, evidence, POCs, cache, mDNS cache). Preserves dictionaries (`lst/`) and rules (`rules/`), and leaves the LLM session (`session.json`) untouched.
