...
class PasswordsView(BaseView):
    ...
    def _build_ui(self):
        ...
        add_btn = tk.Label(
            btn_frame, text="  Add password  ", bg="#222222", fg=BRIGHT,
            font=fonts.view_font(10), relief=tk.RAISED, bd=1,
            padx=15, pady=6,
        )
    ...
    def on_activate(self):
        _dbg(f"[pwds] on_activate")
        self._last = None
        self.after(10, self._poll)

    def on_deactivate(self):
        _dbg(f"[pwds] on_deactivate")
        if self._poll_id:
    ...
    def _poll(self):
        ...
        _dbg(f"[pwds] _poll load_passwords took {time.perf_counter() - t0:.3f}s ({len(items)} items)")
        current = "|".join(items)
        ...
        if not items:
            ...
            self.text.insert(tk.END, "    No passwords stored yet.\n", "muted")
        else:
            for item in items:
    ...
    def _open_add_dialog(self):
        ...
        tk.Label(
            dialog, text="Password:", font=fonts.view_font(11),
            fg=BRIGHT, bg="#111111",
        ).pack(pady=(15, 5))
        ...
        def _save():
            pwd = var.get().strip()
            if pwd:
                credential_db.save_password(pwd)
            dialog.destroy()
