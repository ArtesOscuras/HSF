...
class SSHDetailView(ShellDetailView):
    name = "ssh_detail"
    description = "SSH shell interaction view"
    ...
    def _resize_terminal(self):
        ...
        try:
            ...
            useful_w = max(1, self.terminal.winfo_width() - pad_x)
            useful_h = max(1, self.terminal.winfo_height() - pad_y)
            cols = max(20, useful_w // char_w)
            rows = max(5, useful_h // char_h)
            s = shell_db.get_session(self._sid)
            if s and s["status"] == "connected":
                ch = s.get("ssh_channel")
                if ch:
